...
export default function CategoryUpdate({middleware}){
    ...
    useEffect(() => {
        fetchData("show", "category", setCategory, params.id)
    }, [params]);
    ...
    async function handleSubmit(e){
        ...
        await Api
            .put(`v1/category/${params.id}`, formData, {
                headers: {
    ...
    return (
        <section id="category-create" className="pt-5 mt-5 mb-5">
            <div className="container mt-2">
                <PageHeader title={"Update category"} text={"Update list of product category"} addLink={false}/>
                <form onSubmit={handleSubmit} className="row w-100">
                    <div className="mb-3">
                        <label htmlFor="name" className="form-label fw-bold">Name</label>
                        <input type="text" name="name" id="name" className="form-control" onChange={handleChange} placeholder="Enter Category Name" value={category?.name} />
                    </div>
